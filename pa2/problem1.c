...
void *kMeans(void *threadarg){
	...
	int end = my_data->end;
	// for(int u = 0; u < 6; u++){
	// 	printf("mean value in kMeans, cluster %d is : %f\n", u, m[u]);
	// }
	...
	for(int k = start; k <= end; k++){
		...
		for(int z = 0; z < 6; z++){
			...
			// printf("data: %f. mean: %f, dist: %f\n", (double)my_data->data[k], m[z], dist[z]);
			// printf("distance z: %f \n", dist[z]);
			if(dist[z] < minimum ){
				minimum = dist[z];
				index = z; 
		...
		//printf("sum: %f ", my_data->sum[index]);
		//printf("count: %d\n", my_data->count[index]);
	}
...
int main(int argc, char** argv){
	...
	for(int j = 0;j < 30 ; j++){
		...
		// for(int r = 0; r < 2;r++){
		// 	// printf("thread data: for thread %d \n", r);
		// 	// for(int q = 0; q < 6; q++){
		// 	// 	printf("cluster %d's sum: %f ", q, m[q]);
		// 	// }
		// 	// printf("\n");
		// 	// printf("Cluster %d's mean: %f \n",q, m[q]);
		// }
		//update each mean value
